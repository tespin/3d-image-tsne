...
void ofApp::setup()
{
    ...
    bool encodeIntermediateLayers = settings.value("encode_intermediate_layers", true);
...
void ofApp::encode(const std::filesystem::path& path)
{
    ...
    {
        ofLogError("ofApp::encode") << "Unable to submite image to encoder " << currentWorkerIndex << " : " << path;
    }
}
